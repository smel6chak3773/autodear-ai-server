...
app.post("/api/wallet/:walletType/:ownerId/refund", async (req, res) => {
  const authResult =
    await resolveAuthenticatedUser(req);
  ...
  if (!supabaseServiceRole) {
    console.error(
      "[AUTODEAR][WALLET][REFUND_SERVICE_ROLE_MISSING]"
    );
  ...
  /*
   * Never trust ownerId supplied by the client.
   * The wallet owner must be the authenticated
   * Supabase auth UUID.
   */
  if (ownerId !== authUserId) {
    console.warn(
      "[AUTODEAR][WALLET][REFUND_OWNER_MISMATCH]",
      {
        authUserId,
        ownerId,
        walletType,
  ...
  try {
    ...
    if (refundError) {
      console.error(
        "[AUTODEAR][WALLET][REFUND_RPC_ERROR]",
        {
          ownerId,
          walletType,
          operationKey,
          code:
            refundError.code || null,
          message:
    ...
    if (
      refundResult?.ok !== true
    ) {
      return res.status(500).json({
        ok: false,
        error:
          refundResult?.error ||
          "WALLET_REFUND_INVALID_RESULT",
      });
    }

    console.log(
      "[AUTODEAR][WALLET][REFUND_OK]",
      {
        ownerId,
        walletType,
        operationKey,
        duplicate:
          refundResult?.duplicate === true,
        balance:
          Number(
            refundResult?.balance || 0
          ),
      }
    );
